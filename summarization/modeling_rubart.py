...
"""Copy of modeling_bart from transformers==3.1.0.
 Small edits by AlexanderKazakov to incorporate deeppavlov/rubert as BART encoder"""
import math
import random
import warnings
...
class EncoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.self_attn_layer_norm = LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.fc1 = nn.Linear(self.embed_dim, config.encoder_ffn_dim)
        self.fc2 = nn.Linear(config.encoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        assert not config.scale_embedding
        ...
        self.layernorm_embedding = LayerNorm(embed_dim, eps=config.layer_norm_eps) if config.normalize_embedding else nn.Identity()
        assert not config.normalize_before
        # mbart has one extra layer_norm
        self.layer_norm = None

        self.token_type_embeddings = nn.Embedding(2, config.d_model)  # to keep identity with RuBert embeddings
...
class DecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.encoder_attn_layer_norm = LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        self.layernorm_embedding = LayerNorm(config.d_model, eps=config.layer_norm_eps)
        assert not config.add_final_layer_norm
        self.layer_norm = None
        # to keep identity with RuBert embeddings
    ...
    def forward(
        ...
        **unused,
        ...
        if labels is not None:
            ...
            assert loss_fct.ignore_index == -100
